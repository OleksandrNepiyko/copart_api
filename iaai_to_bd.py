...
START_ID = 300000
END_ID = 500000  # На другому ноуті зміни на 200000, на віртуалці на 300000

BATCH_SIZE = 500
# TEST_LIMIT = 100   # ЗАКОМЕНТУЙ ЦЕЙ РЯДОК ДЛЯ ПОВНОГО ЗАПУСКУ: # TEST_LIMIT = None
...
def parse_iaai_date(date_str):
    if not date_str:
        return None

    date_str = str(date_str).strip()

    # Формат 1: /Date(1234567890000)/
    match = re.search(r'/Date\((\d+)(?:[+-]\d+)?\)/', date_str)
    if match:
        try:
            timestamp = int(match.group(1)) / 1000.0
            dt = datetime.fromtimestamp(timestamp)
            if dt.year < 1920: return None # Відсікаємо 1899
            return dt.strftime('%Y-%m-%d %H:%M:%S')
        except: return None

    # Формат 2: "12/31/1899 6:00:00 AM +00:00"
    try:
        # Відрізаємо часовий пояс для стабільності
        date_part = date_str.split(' +')[0]
        # Парсимо: Місяць/День/Рік Година:Хв:Сек AM/PM
        dt = datetime.strptime(date_part, '%m/%d/%Y %I:%M:%S %p')

        # Якщо дата 1899 року — повертаємо None, бо це пусте значення в IAAI
        if dt.year < 1920:
            return None

        return dt.strftime('%Y-%m-%d %H:%M:%S')
    except Exception as e:
        # Якщо не вдалося розпарсити — ігноруємо помилку
        return None
...
def parse_iaai_to_pydantic(lot_obj: Dict[str, Any]) -> AuctionLotParsedData:
    ...
    raw_date = attributes.get('AuctionDateTime') or specs.get('auction_date_and_time')
    sale_date_time_val = parse_iaai_date(raw_date)
    ensurance_name_val = specs.get('ensurance') or attributes.get('ProviderName')
